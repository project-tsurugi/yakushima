...
TEST_F(kt, test3) { // NOLINT
  ...
  for (std::size_t i = 0; i < ary_size; ++i) {
    std::pair<char *, std::size_t> tuple =
        get<char>(test_storage_name, std::string_view(k.at(i)));
...
TEST_F(kt, test4) { // NOLINT
  for (std::size_t h = 0; h < 1; ++h) {
    ...
    for (std::size_t i = 0; i < ary_size; ++i) {
      ASSERT_EQ(status::OK, put(token, test_storage_name, std::get<0>(kv[i]),
    ...
    for (std::size_t i = 0; i < ary_size; ++i) {
      ...
      constexpr std::size_t size_index = 1;
      std::pair<char *, std::size_t> tuple =
          get<char>(test_storage_name, std::get<0>(kv[i]));
  ...
  for (std::size_t i = 0; i < ary_size; ++i) {
    ...
    if (i == key_slice_length - 1) {
      ...
      ASSERT_EQ(dynamic_cast<border_node *>(
                ...
                8);
      ASSERT_EQ(dynamic_cast<border_node *>(
                    dynamic_cast<interior_node *>(ti->load_root_ptr())->get_child_at(1))
                    ->get_permutation_cnk(),
                8);
      ...
      ASSERT_EQ(dynamic_cast<border_node *>(
                ...
                8);
      ASSERT_EQ(dynamic_cast<border_node *>(
                    dynamic_cast<interior_node *>(ti->load_root_ptr())->get_child_at(2))
                    ->get_permutation_cnk(),
                8);
    } else if ((i > key_slice_length + (key_slice_length / 2) + 1) &&
               (i <
                key_slice_length + (key_slice_length / 2 + 1) * (key_slice_length - 1)) &&
...
TEST_F(kt, test12) { // NOLINT
  ...
  scan<char>(test_storage_name, k, scan_endpoint::EXCLUSIVE, k4, scan_endpoint::EXCLUSIVE,
             tuple_list);
