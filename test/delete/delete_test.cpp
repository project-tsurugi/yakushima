...
TEST_F(dt, one_border) { // NOLINT
    ...
    for (std::size_t i = 0; i < ary_size; ++i) {
        ASSERT_OK(remove(token, test_storage_name, k.at(i)));
...
TEST_F(dt, one_border_shuffle) { // NOLINT
    for (std::size_t h = 0; h < 1; ++h) {
        ...
        for (std::size_t i = 0; i < ary_size; ++i) {
            ASSERT_OK(put(token, test_storage_name, std::get<0>(kv.at(i)),
                          std::get<1>(kv.at(i)).data(),
                          std::get<1>(kv.at(i)).size()));
        }

        for (std::size_t i = 0; i < ary_size; ++i) {
            ASSERT_OK(remove(token, test_storage_name, std::get<0>(kv.at(i))));
            auto* br = dynamic_cast<border_node*>(ti->load_root_ptr());
            if (i != ary_size - 1) {
                ASSERT_EQ(br->get_permutation_cnk(), ary_size - i - 1);
            }
...
TEST_F(dt, two_interi) { // NOLINT
    ...
    ASSERT_EQ(n_in_bn - 1,
              dynamic_cast<interior_node*>(in->get_child_at(0))->get_n_keys());
