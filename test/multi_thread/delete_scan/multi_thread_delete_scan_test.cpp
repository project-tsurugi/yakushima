/**
 * @file multi_thread_delete_scan_test.cpp
 */
...
auto make_key(std::size_t i, int key_width) {
    std::ostringstream ss;
    ss << std::hex << std::setw(key_width) << std::setfill('0') << i;
    return ss.str();
}
...
static void test_sub(std::size_t num_scan_th, const std::bitset<data_size>& delete_target, std::size_t repeats, int key_width) {
    // setup
...
TEST_F(multi_thread_delete_scan_test, delete_L1_border_leaf_during_scan_all) { // NOLINT
    std::bitset<data_size> delete_target;
    for (std::size_t i = 0; i < data_size; i++) {
        delete_target.set(i, true); // all
    }
    test_sub(5, delete_target, 1, 10); // Layer 1
}
