...
TEST_F(mtpdst, one_interior) { // NOLINT
  /**
   * concurrent put/delete/scan in the state between none to split of interior, which is
   * using shuffled data.
   */
      ...
      static void work(std::size_t th_id, std::size_t max_thread) {
        ...
        for (std::size_t i = (ary_size / max_thread) * th_id;
          ...
          if (i <= INT8_MAX) {
            ...
            kv.emplace_back(
                std::make_pair(std::string(i / INT8_MAX, static_cast<char>(INT8_MAX)) +
