...
TEST_F(st, scan_against_single_put_null_key_to_one_border) { // NOLINT
  ...
  auto verify_exist = [&tup_lis, &nv, &v]() {
    if (tup_lis.size() != 1)
      return false;
    if (tup_lis.size() != nv.size())
      return false;
    if (std::get<2>(tup_lis.at(0)) != v.size())
      return false;
  ...
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INCLUSIVE, "",
...
TEST_F(st, scan_against_single_put_non_null_key_to_one_border) { // NOLINT
  ...
  ASSERT_EQ(verify_no_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INCLUSIVE, k,
                                   scan_endpoint::INF, tup_lis, &nv));
  ASSERT_EQ(verify_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INCLUSIVE, k,
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ...
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INF, "",
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(verify_no_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INCLUSIVE, "",
                                   scan_endpoint::INF, tup_lis, &nv));
  ASSERT_EQ(verify_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INCLUSIVE, "",
                                   scan_endpoint::INCLUSIVE, tup_lis, &nv));
  ...
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INCLUSIVE, "",
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(verify_no_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::EXCLUSIVE, "",
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(verify_no_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::EXCLUSIVE, "",
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ...
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::EXCLUSIVE, "",
                                   scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(verify_no_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INF, k,
                                   scan_endpoint::INF, tup_lis, &nv));
  ASSERT_EQ(verify_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INF, k,
                                   scan_endpoint::INCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(verify_exist(), true);
  ASSERT_EQ(status::ERR_BAD_USAGE, scan<char>(test_storage_name, k, scan_endpoint::INF, k,
                                              scan_endpoint::EXCLUSIVE, tup_lis, &nv));
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INCLUSIVE, k,
                                   scan_endpoint::INF, tup_lis, &nv));
  ASSERT_EQ(verify_exist(), true);
  ASSERT_EQ(status::OK, scan<char>(test_storage_name, k, scan_endpoint::INCLUSIVE, k,
                                   scan_endpoint::INCLUSIVE, tup_lis, &nv));
...
TEST_F(st, scan_multiple_same_null_char_key_1) { // NOLINT
  ...
  for (std::size_t i = 0; i < ary_size; ++i) {
    scan<char>(test_storage_name, "", scan_endpoint::INF, std::string_view(k.at(i)),
               scan_endpoint::INCLUSIVE, tuple_list);
    for (std::size_t j = 0; j < i + 1; ++j) {
      ASSERT_EQ(
          memcmp(std::get<v_index>(tuple_list.at(j)), v.at(j).data(), v.at(j).size()), 0);
    }
  }

  for (std::size_t i = ary_size - 1; i > 1; --i) {
    std::vector<std::pair<node_version64_body, node_version64 *>> nv;
    scan<char>(test_storage_name, std::string_view(k.at(i)), scan_endpoint::INCLUSIVE, "",
    ...
    for (std::size_t j = i; j < ary_size; ++j) {
      ASSERT_EQ(
          memcmp(std::get<v_index>(tuple_list.at(j - i)), v.at(j).data(), v.at(j).size()),
          0);
    }
  }
  ASSERT_EQ(leave(token), status::OK);
