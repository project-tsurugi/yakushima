...
TEST_F(st, one_border) { // NOLINT
    ...
    ASSERT_EQ(tuple_list.size(), 3);
    ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INF, "", scan_endpoint::INF, tuple_list, nullptr, 1));
    ASSERT_EQ(tuple_list.size(), 1);
    ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INF, "", scan_endpoint::INF, tuple_list, nullptr, 2));
