...
TEST_F(st, one_border) { // NOLINT
  ...
  ASSERT_EQ(status::OK,
            put(token, test_storage_name, std::string_view(k1), v.data(), v.size()));
  ASSERT_EQ(status::OK,
            put(token, test_storage_name, std::string_view(k2), v.data(), v.size()));
  ASSERT_EQ(status::OK,
            put(token, test_storage_name, std::string_view(k3), v.data(), v.size()));
