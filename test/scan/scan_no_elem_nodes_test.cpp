...
TEST_F(st, scan_three_border_one_border) { // NOLINT
    ...
    ASSERT_EQ(status::OK, scan<char>(test_storage_name, "", scan_endpoint::INF, "", scan_endpoint::INF, tup, &nv));
    auto delete_range = [&token](char begin, char end) {
        for (char i = begin; i <= end; ++i) {
            char c = i;
            ASSERT_EQ(status::OK, remove(token, test_storage_name, std::string_view(&c, 1)));
        }
    };
    ...
    char begin = 1;                                                 // NOLINT
    char end = 24;                                                  // NOLINT
    ASSERT_EQ(status::OK, scan<char>(test_storage_name, std::string_view(&begin, 1), scan_endpoint::INCLUSIVE, std::string_view(&end, 1), scan_endpoint::INCLUSIVE, tup, &nv));
