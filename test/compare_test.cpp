...
TEST_F(ct, compareData) { // NOLINT
  ...
  ASSERT_EQ(s1 < s2, std::string_view(reinterpret_cast<char *>(&k1),
                                      sizeof(std::uint64_t)) < // NOLINT
                         std::string_view(reinterpret_cast<char *>(&k2),
                                          sizeof(std::uint64_t))); // NOLINT
...
TEST_F(ct, compareStringView) { // NOLINT
  ...
  key_length.at(1) = 1;
  ASSERT_EQ(
      (std::string_view(reinterpret_cast<char *>(&key_slice.at(0)),
                        key_length.at(0)) < // NOLINT
