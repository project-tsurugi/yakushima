...
TEST_F(kvs_test, init) {
  ...
  ASSERT_EQ(status::OK, masstree_kvs::put(token, std::string_view(k), v.data(), v.size()));
...
TEST_F(kvs_test, delete_against_put_until_first_split_of_interior_node) {
  Token token;
  ASSERT_EQ(masstree_kvs::enter(token), status::OK);
  std::size_t ary_size;
  if (base_node::key_slice_length % 2) {
  ...
  ASSERT_EQ(n_in_bn-1, dynamic_cast<interior_node*>(dynamic_cast<interior_node*>(base_node::get_root())->get_child_at(0))->get_n_keys());
