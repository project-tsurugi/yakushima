...
TEST_F(st, simple_operate_some_storage) { // NOLINT
  ...
  ASSERT_EQ(status::OK,
            scan(st1, "", scan_endpoint::INF, "", scan_endpoint::INF, tuple_list));
  ASSERT_EQ(tuple_list.size(), 3);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(0)).data(), k1.data(), k1.size()),
            0);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(1)).data(), k2.data(), k1.size()),
            0);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(2)).data(), k3.data(), k1.size()),
            0);
  ASSERT_EQ(status::OK,
            scan(st2, "", scan_endpoint::INF, "", scan_endpoint::INF, tuple_list));
  ASSERT_EQ(tuple_list.size(), 3);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(0)).data(), k4.data(), k1.size()),
            0);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(1)).data(), k5.data(), k1.size()),
            0);
  ASSERT_EQ(memcmp(std::get<key_index>(tuple_list.at(2)).data(), k6.data(), k1.size()),
            0);
