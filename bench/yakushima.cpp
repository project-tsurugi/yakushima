...
static void check_flags() {
  ...
  if (FLAGS_instruction != "insert" && FLAGS_instruction != "put" &&
    ...
    LOG(FATAL)
        << "The instruction option must be insert or put or get. The default is insert.";
  }
    ...
    for (std::size_t i = 0; i < std::thread::hardware_concurrency(); ++i) {
      if (i != std::thread::hardware_concurrency() - 1) {
        thv.emplace_back(
            ...
            FLAGS_get_initial_record / std::thread::hardware_concurrency() * i,
        ...
        thv.emplace_back(S::parallel_build_worker,
                         FLAGS_get_initial_record / std::thread::hardware_concurrency() *
                             i,
