...
public:
  static void epoch_thread() {
    for (;;) {
      ...
      for (;;) {
         ...
         */
        if (kEpochThreadEnd.load(std::memory_order_acquire))
          break;
      }
  ...
  static void gc_thread() {
    for (;;) {
      ...
      if (kGCThreadEnd.load(std::memory_order_acquire))
        break;
    }
  }
  ...
  static void set_gc_thread_end() { kGCThreadEnd.store(true, std::memory_order_release); }
