...
public:
    ...
    static void invoke_gc_thread() {
        kGCThread = std::thread(gc_thread);
    }
...
private:
    alignas(CACHE_LINE_SIZE) static inline std::atomic<bool> kEpochThreadEnd{false}; // NOLINT : can't become constexpr
    static inline std::thread kEpochThread;                                          // NOLINT : can't become constexpr
    alignas(CACHE_LINE_SIZE) static inline std::atomic<bool> kGCThreadEnd{false};    // NOLINT : can't become constexpr
