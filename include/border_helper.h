...
insert_lv(tree_instance *ti, border_node *const border, std::string_view key_view,
  ...
  if (cnk == key_slice_length) {
    ...
    border_split<interior_node, border_node>(
        ti, border, key_view, value_ptr, created_value_ptr, arg_value_length, value_align,
    ...
    border->insert_lv_at(border->assign_empty_slot(), key_view,
                         {value_ptr, arg_value_length, value_align}, created_value_ptr,
                         rank);
...
border_split(tree_instance *ti, border_node *const border, std::string_view key_view,
  ...
  for (std::size_t i = remaining_size; i < key_slice_length; ++i) {
    ...
    std::size_t src_index{
        border->get_permutation().get_index_of_rank(remaining_size)}; // this is tricky.
  ...
  if (key_length == 0 || ret_memcmp < 0 ||
    ...
    border->insert_lv_at(border->assign_empty_slot(), key_view,
                         {value_ptr, value_length, value_align}, created_value_ptr, rank);
  } else {
    /**
