...
static void border_split(tree_instance* ti, border_node* const border,
    ...
    int ret_memcmp{
            memcmp(&key_slice, &new_border->get_key_slice_ref().at(0),
                   std::min<key_length_type>({key_length,
    ...
    if (p == nullptr) {
        ...
        p->version_unlock();
        ti->root_unlock();
VLOG(10) << "split root b:" << border << " p:" << p << " nb:" << new_border;
        return;
    }
    ...
    if (p->get_version_border()) {
        ...
        p->version_unlock();
VLOG(10) << "split b:" << border << " pi:" << pi << " pb:" << pb << " nb:" << new_border;
        return;
    }
    /**
...
VLOG(10) << "split b:" << border << " p:" << pi << " nb:" << new_border;
