 ...
 */
template <typename T> static T loadRelaxed(T &ptr) {
 ...
 */
template <typename T> static T loadAcquireN(T &ref) { // NOLINT
  return __atomic_load_n(&ref, __ATOMIC_ACQUIRE);     // NOLINT
}

template <class type> void loadAcquire(type *ptr, type *ret) {
 ...
 */
template <typename T, typename T2> static void storeRelaxed(T &ptr, T2 val) {
 ...
 */
template <typename T, typename T2> static void storeReleaseN(T &ptr, T2 val) {
  __atomic_store_n(&ptr, static_cast<T>(val), __ATOMIC_RELEASE); // NOLINT
}

template <class type> void storeRelease(type *ptr, type *val) {
