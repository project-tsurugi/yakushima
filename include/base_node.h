...
public:
  ...
  void display_base() {
    ...
    for (std::size_t i = 0; i < key_slice_length; ++i) {
      std::cout << "key_slice_[" << i << "] : " << std::to_string(get_key_slice_at(i))
                << std::endl;
      std::cout << "key_length_[" << i << "] : " << std::to_string(get_key_length_at(i))
                << std::endl;
    }
  }

  [[nodiscard]] const std::array<key_length_type, key_slice_length> &
  get_key_length_ref() const { return key_length_; }
  ...
  get_key_slice_ref() const { return key_slice_; }
