...
public:
   ...
   */
  template <class interior_node, class border_node> void fin() {
    // for cache
    ...
    while (!node_container_.empty()) {
      ...
      while (!node_container_.try_pop(elem))
        continue;
    ...
    while (!value_container_.empty()) {
      ...
      while (!value_container_.try_pop(elem))
        continue;
   ...
   * @attention Use a template class so that the dependency does not cycle.
   */
  template <class interior_node, class border_node> void gc() {
   ...
   * @attention Use a template class so that the dependency does not cycle.
   */
  template <class interior_node, class border_node> void gc_node() {
