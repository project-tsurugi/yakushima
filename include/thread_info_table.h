...
public:
   ...
   * @param[out] token If the return value of the function is status::OK, then the token
   * is the acquired session.
   ...
   * @pre global epoch is not yet functional because it assigns 0 to begin_epoch as the
   * initial value.
   * @return void
   */
  static void init() {
  ...
  /**
   * @details When @a token points to an invalid memory location, an error occurs if @a
   * token is referenced. To avoid this, it scans the table. So if @token is invalid one,
   * return status::WARN_INVALID_TOKEN.
   * @tparam interior_node Class information is given at compile time to eliminate the
...
private:
   ...
   */
  static inline std::array<thread_info, YAKUSHIMA_MAX_PARALLEL_SESSIONS>
