...
template <class ValueType>
static status scan(
    base_node *const root, const std::string_view l_key, const scan_endpoint l_end,
...
retry:
  ...
  for (;;) {
    check_status =
        scan_border<ValueType>(&bn, l_key, l_end, r_key, r_end, tuple_list, check_v,
    ...
    if (check_status == status::OK_RETRY_AFTER_FB) {
      ...
      if (check_v.get_vsplit() != re_check_v.get_vsplit() || re_check_v.get_deleted()) {
        return status::OK_RETRY_AFTER_FB;
      }
...
template <class ValueType>
static status scan_border(
    border_node **const target, const std::string_view l_key, const scan_endpoint l_end,
    const std::string_view r_key, const scan_endpoint r_end,
    std::vector<std::tuple<std::string, ValueType *, std::size_t>> &tuple_list,
    node_version64_body &v_at_fb,
    std::vector<std::pair<node_version64_body, node_version64 *>> *const node_version_vec,
...
retry:
  ...
  for (std::size_t i = 0; i < perm.get_cnk(); ++i) {
    ...
    status check_status =
        scan_check_retry(bn, v_at_fb, tuple_list, tuple_pushed_num, node_version_vec);
    ...
    if (kl > sizeof(key_slice_type)) {
      ...
      auto in_range = [&full_key, &tuple_list, &vp, &vsize, &node_version_vec, &v_at_fb,
                       &node_version_ptr, &tuple_pushed_num, max_size]() {
      ...
      if (l_end != scan_endpoint::INF) {
        ...
        if (l_cmp > 0 ||
            (l_cmp == 0 && (l_key.size() > kl ||
                            (l_key.size() == kl && l_end == scan_endpoint::EXCLUSIVE)))) {
          continue;
        }
      }
      // pass left endpoint.
      ...
      if (tuple_pushed_num == 0 && node_version_vec != nullptr) {
        /**
         * Since it is a rightmost node included in the range, it is included in the
         * phantom verification. However, there were no elements included in the range.
         */
  ...
  if (tuple_pushed_num == 0 && node_version_vec != nullptr) {
    /**
     * Since it is a leftmost node included in the range, it is included in the phantom
     * verification. However, there were no elements included in the range.
     */
    node_version_vec->emplace_back(std::make_pair(v_at_fb, bn->get_version_ptr()));
  }
