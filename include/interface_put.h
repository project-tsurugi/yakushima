...
retry_fetch_lv:
  ...
  link_or_value *lv_ptr =
      target_border->get_lv_of(key_slice, key_slice_length, v_at_fetch_lv, lv_pos, &rank);
  /**
  ...
  if (lv_ptr == nullptr) {
    ...
    insert_lv<interior_node, border_node>(ti, target_border, traverse_key_view, value_ptr,
                                          reinterpret_cast<void **>(created_value_ptr),
    ...
    if (key_slice_length <= sizeof(key_slice_type)) {
      ...
      if (old_value.get_body() != nullptr) {
        thin->get_gc_info().push_value_container(
            {thin->get_begin_epoch(), old_value.get_body(), old_value.get_len(),
             old_value.get_align()});
      }
      return status::OK;
    }
